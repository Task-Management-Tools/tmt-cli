...
from .parser import DictParser, TMTConfigError, TMTConfigErrorsList, check_error
...
def parse_solution(data: Any) -> Solution | TMTConfigErrorsList:
    # When solution type is unclear, we can
    ...
    return parser.errors or Solution(
        time_limit_sec=check_error(time_limit_sec),
        memory_limit_mib=check_error(memory_limit_mib),
        output_limit_mib=check_error(output_limit_mib),
        compilation=check_error(compilation),
        execution=check_error(execution),
    )
