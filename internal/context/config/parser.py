...
class TMTConfigParsingError(AssertionError):
    """
    Raised by unwrap (below) when arguments are error-typed value.
    ...
    def __init__(self, err: TMTConfigError | TMTConfigErrorsList):
        match err:
            case TMTConfigError():
                super().__init__(f"unwrap: value is error-typed: {err.what}")
            case TMTConfigErrorsList():
                errs = "; ".join(e.what for e in err)
                super().__init__(f"unwrap: value is error-typed: {errs}")


def unwrap(t: T | TMTConfigError | TMTConfigErrorsList) -> T:
