...
from .operations import CopyFileOperation, CustomFileOperation, RegexCopyOperation, ExternalFileOperation
...
class FolderFormatExporter:
    ...
    def export(self, formatter: Formatter, context: TMTContext, create_zip: bool = True) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            if not create_zip:
                ...
                if output_dir.exists():
                    formatter.println(formatter.ANSI_RED, f"Error: path {self.output_path} already exists.", formatter.ANSI_RESET)
                    return
                output_dir.mkdir();
            else:
