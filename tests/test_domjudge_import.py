    ...
    try:
        ...
        for pattern in problem.scan_messages:
            if not any(re.fullmatch(pattern, msg) for msg in msgs):
                assert False, (
                    f"pattern {pattern} did not match any of the messages: {msgs}"
                )
        for pattern in problem.scan_not_messages:
            if any(re.fullmatch(pattern, msg) for msg in msgs):
                matched_msgs = filter(lambda x: re.fullmatch(pattern, x), msgs)
                matched_msg_fmt = ", ".join(matched_msgs)
                assert False, (
                    f"negative pattern {pattern} matched messages: {matched_msg_fmt}"
                )
