...
int main()
{
    ...
    mapping[std::pair("subtask1", 0)] = wrong;
    mapping[std::pair("subtask1", 1)] = wrong;
    mapping[std::pair("subtask2", 0)] = partial;
    mapping[std::pair("subtask2", 1)] = partial;
    mapping[std::pair("subtask2", 2)] = partial;
    mapping[std::pair("subtask3", 0)] = timeout;
    mapping[std::pair("subtask3", 1)] = timeout_a_bit;
    mapping[std::pair("subtask4", 0)] = memory;
    mapping[std::pair("full", 0)] = runtime_error;
    mapping[std::pair("full", 1)] = correct;
